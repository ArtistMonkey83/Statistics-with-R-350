...
answer<-sum(number_to_add<-sample(dice,10000,replace= TRUE),number_to_Be_added<-sample(dice,10000,replace= TRUE))

die_1<-sample(1:6,10000,replace=TRUE)
die_2<-sample(1:6,10000,replace=TRUE)

sim_results<-die_1 +die_2
sim_results[1:10]
...
E<-sim_results==6
E[1:20]
mean(E) #changes everything to zeros and ones because it is true and false
...
a<-sample(sample_space,10000,replace = TRUE,prob = color_prob)

green_ms<- a== "green"
(mean(green_ms))

1-mean(green_ms =="green")
...
results<-sample(sample_space,10000,replace= TRUE, prob= color_prob)
#1-mean(results=="green")

mean(results=="yelloww" | results == "orange" | results == "red")
...
dice_uno<-sample((dice), size=7,replace=TRUE) #dice 2
...
results<-replicate(1000,{dice<-1:6 #declare a dice that is six sided

dice_dos<-sample((dice), size=7,replace=TRUE) #dice 2
dice_uno<-sample((dice), size=7,replace=TRUE) #dice 2

difference<-(dice_uno-dice_dos <3 | dice_dos-dice_uno <3)} )

(mean(results))


#########teachers way to do it
die1<-sample(1:6,1)
die2<-sample(1:6,1)
diff_3<- abs(die1-die2)< 3



###step2
results_diff3<-replicate(10000,{die1<-sample(1:6,1)
die2<-sample(1:6,1)
diff_3<- abs(die1-die2)< 3})
...
results<-replicate(10, {coin<-sample(0:1,1)})
...
###teachers way
#you could do this..... toss<-c("heads","tails")
toss<-c(0,1)
...
##step 2
results<- replicate(10000,{toss<-c(0,1)
results_toss<-sample(toss,10,replace=TRUE)
results_toss[8:10]== 1
sum(results_toss[8:10])==3 })
...
#results<-replicate(10000,
      #{toss<-c(0,1)
      #results_toss<-sample(toss,10,replace=TRUE)            sum(results_toss[8:10])})
...
```{r}
##setting up trial variables for two people
blood_type<-c('A','AB','B','O')
probability<-c(0.40,0.04,0.11,0.45)
p1<- sample(blood_type,10000,prob=probability,replace=TRUE)
p2<- sample(blood_type,10000,prob=probability,replace=TRUE)
duplicate <-p1==p2
mean(duplicate)
```
...
```{r}
results<-replicate(10000,{days<-1:365
myBday<-23
randoGuy<-sample(days,199,replace= TRUE)
birthday_twins<-sum(myBday == randoGuy[1:199])>0
})
