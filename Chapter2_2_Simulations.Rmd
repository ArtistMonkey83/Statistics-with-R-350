...
#balls<-1:100
#urns<-1:20
#(filling_Urns<- sample(urns,20sample(balls,100))
#empty_Urns<-sum(filling_Urns ==0)
...
#length(unique(urns)) #this will return 20 if no urns are empty
results<-sample(ballfinal < 20)
mean(ballfinal)
```
...
#p2<-sum(sample(deck,1),sample(deck,1))
  blackjack <-21
...
snackBag<-replicate(100000,{
  pickedMs<-sample(colors,30,replace = TRUE) #this has 30 randomly selected M&Ms

#We need to know if there are  at least 9 blue and 6 orange
targetMs<-sum(pickedMs=="blue") >= 9 & sum(pickedMs =="orange")>=6

#saving the bags that actually made the cut the ones that have the correct ammount
targetMs
